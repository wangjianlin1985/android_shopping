...
public class NoticeAction extends BaseAction {
    ...
    public String QueryNoticeOutputToExcel() { 
        ...
        String[] headers = { "记录编号","公告标题","公告内容","发布日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<noticeList.size();i++) {
        	Notice notice = noticeList.get(i); 
        	dataset.add(new String[]{notice.getNoticeId() + "",notice.getTitle(),notice.getContent(),new SimpleDateFormat("yyyy-MM-dd").format(notice.getPublishDate())});
        }
        /*
